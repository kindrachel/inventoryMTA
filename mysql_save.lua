...
function readTable(player,tableName,synch,...)
	...
	if(mysql_num_rows(result) ~= 0) then
		...
		for index,value in pairs(pInfo) do
			if(tonumber(value)) then
				setData(player,tostring(tableName),tostring(index),tonumber(value),synch)
			else
				setData(player,tostring(tableName),tostring(index),value,synch)
			end
		end
	else
